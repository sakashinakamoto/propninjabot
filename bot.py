...
amp = math.sqrt(max(prob, 0.01))
boost = 0.04 * math.sin(math.pi * amp)
return round(min((amp + boost) ** 2, 0.99), 4)
...
b = odds - 1.0
k = (prob * b - (1.0 - prob)) / b if b > 0 else 0
...
win_b = 1.0 - win_a
proj_a = sum(sa_list) / NUM_SIMS
proj_b = sum(sb_list) / NUM_SIMS
diffs = [a - b for a, b in zip(sa_list, sb_list)]
spread = sum(diffs) / NUM_SIMS
var = sum((d - spread) ** 2 for d in diffs) / NUM_SIMS
spread_std = math.sqrt(var)
sa_sorted = sorted(sa_list)
sb_sorted = sorted(sb_list)
ci_a = [sa_sorted[int(0.05 * NUM_SIMS)], sa_sorted[int(0.95 * NUM_SIMS)]]
ci_b = [sb_sorted[int(0.05 * NUM_SIMS)], sb_sorted[int(0.95 * NUM_SIMS)]]
imp_a = 1.0 / odds_a
imp_b = 1.0 / odds_b
...
ev_pick = team_a
k_stake = round(k_a * 100, 2)
elif ev_b > 0:
ev_pick = team_b
k_stake = round(k_b * 100, 2)
else:
ev_pick = ‘No +EV’
k_stake = 0.0
return {
...
‘ev_pick’: ev_pick, ‘k_stake’: k_stake,
...
msg += ‘Kelly Stake: $’ + str(r[‘k_stake’]) + ’ per $100\n’
msg += ‘EV ’ + a + ‘: $’ + str(r[‘ev_a’])
...
tickers = [‘NBA’, ‘NFL’, ‘MLB’, ‘NHL’, ‘SOCCER’, ‘UFC’, ‘GOLF’, ‘TEN’, ‘KXNBA’, ‘KXNFL’, ‘KXMLB’, ‘KXNHL’, ‘EPL’]
sport_map = {
‘NBA’: ‘NBA’, ‘KXNBA’: ‘NBA’,
‘NFL’: ‘NFL’, ‘KXNFL’: ‘NFL’,
‘MLB’: ‘MLB’, ‘KXMLB’: ‘MLB’,
...
odds_h = 1 + (100 / abs(oh)) if oh < 0 else 1 + (oh / 100)
odds_a = 1 + (100 / abs(oa)) if oa < 0 else 1 + (oa / 100)
except Exception:
pass
matchups.append((home, away, sport, odds_h, odds_a))
except Exception as e:
...
msg += ‘––––––––––––––\n’
msg += ‘Live: PrizePicks + Kalshi 24hr\n’
